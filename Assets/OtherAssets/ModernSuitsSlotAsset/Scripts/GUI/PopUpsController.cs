...
namespace Mkey
{
    ...
    public class PopUpsController : MonoBehaviour
    {
        ...
        public void CloseButton_click()
        {
            ...
            MasterAudio.PlaySound("PopUpButton");
        ...
        public void ShowWindow()
        {
            RefreshWindow();
            //if (SoundMasterController.Instance) SoundMasterController.Instance.SoundPlayPopUp(0.2f, null);
            MasterAudio.PlaySoundAndForget("PopUpButton");
            Debug.Log("PopUpButton");
            GetComponent<GuiFader_v2>().FadeIn(0, () =>
            {
                SetControlActivity(true);
                showed = true;
                if (openDelelegate != null) openDelelegate(this);
            });
        }
        ...
        public void PopUpInit(Action<PopUpsController> openDel, Action<PopUpsController> closeDel)
        {
            ...
            {
                openDelelegate = new Action<PopUpsController>((pU) => { });
            }

            if (closeDel != null) closeDelegate = closeDel;
            else
            {
                closeDelegate = new Action<PopUpsController>((pU) => { });
            }
        }
