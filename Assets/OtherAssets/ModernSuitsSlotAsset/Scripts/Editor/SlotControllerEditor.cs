...
namespace Mkey
{
    ...
    public class SlotControllerEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            ShowPropertiesBoxFoldOut("Main references: ", new string[] { "menuController" }, ref showRef, false);
           ...
           // serializedObject.ApplyModifiedProperties();
            ...
            ShowPropertiesBoxFoldOut("Feature Mode: ", new string[] {
                "featureMultiply",
                "featureMultiplyText",
            }, ref showFeature, false);
            
            // ShowFeature("Feature Mode", ref showFeature);
            #endregion spaecial major
        ...
        private void ShowMajorChoise(string bName, ref bool fOut)
        {
            ...
            if (fOut = EditorGUILayout.Foldout(fOut, bName))
            {
              ...
              //  ShowFreeSpinChoiseLO(sChoise);
                ...
                EditorGUILayout.PropertyField(serializedObject.FindProperty("wildPayTable"), true);
        ...
        private void ShowWildChoiseLO(string[] sChoise)
        {
            ...
            if (slotController.useScatter)
            {
                ShowProperties(new string[] { "wild_multiply" }, false);
        ...
        private void ShowScatterChoiseLO(string[] sChoise)
        {
            ...
            if (slotController.useScatter)
            {
                ShowProperties(new string[] { "scatterWildSub" }, false);
                
                ShowProperties(new string[] { "scatterFollowSeq" }, false);
            }
        }
        ...
        private void ShowPayLine(string[] choises, Rect rect, int count, float width, float height, int index, bool isActive, bool isFocused, PayLine pLine)
        {
            ...
            EditorGUI.LabelField(new Rect(dx, rect.y, w, h), "Rev Win: ");
            dx += w;
            w = 30;
            
            EditorGUI.PropertyField(new Rect(dx, rect.y, w, h),
                        element.FindPropertyRelative("reverseWin"), GUIContent.none);
            dx += w; 
            w = 70;
            EditorGUI.LabelField(new Rect(dx, rect.y, w, h), "Wild Multi: ");
            dx += w;
            w = 30;
            
            EditorGUI.PropertyField(new Rect(dx, rect.y, w, h),
                        element.FindPropertyRelative("wildMulti"), GUIContent.none);
            dx += w;
            w = 50;
            ...
            dx += w; w = 105;
            EditorGUI.LabelField(new Rect(dx, rect.y, w, h), "TotalBetMult:");
            dx += w; w = 40;
            EditorGUI.PropertyField(new Rect(dx, rect.y, w, h),
                        element.FindPropertyRelative("totalBetMult"), GUIContent.none);
            dx += w; w = 105;
            int evCounts = 0;
            ...
            dx += w; w = 105;
            string eS = (evCounts > 0) ? "specialEffect(" + evCounts + "):" : "specialEffect :";
            EditorGUI.LabelField(new Rect(dx, rect.y, w, h), eS);
            dx += w; w = 100;
            EditorGUI.PropertyField(new Rect(dx, rect.y, w, h),
                        element.FindPropertyRelative("specialEffect"), GUIContent.none);
