...
namespace Mkey
{
    ...
    public class LineBehaviorEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            ShowPropertiesBox("", new string[] { "typeOfLine" }, false);

            switch (lineBehavior.typeOfLine)
            {
                ...
                case TypeOfWinningLine.line:
                    {
                        ...
                        if (lineBehavior.lineRender == null)
                        {
                            LineRenderer lr = lineBehavior.gameObject.GetComponent<LineRenderer>();
                            if (lr == null)
                                lr = lineBehavior.gameObject.AddComponent<LineRenderer>();
                            lineBehavior.lineRender = lr;
                        }

                        

                        if(lineBehavior.spirteLineScript == null)
                        {
                            SpriteLine sl = lineBehavior.gameObject.GetComponent<SpriteLine>();
                            if(sl == null)
                                sl = lineBehavior.gameObject.AddComponent<SpriteLine>();
                            lineBehavior.spirteLineScript = sl;
                        }

                        EditorGUILayout.BeginVertical("box");
                        EditorGUI.indentLevel += 1;
                        EditorGUILayout.Space();
                        ShowProperties("Line: ", new string[] { "lineMaterial", "lineColor", "lineFlashingSpeed", "lineRendererWidth", "typeOfBoxWinning", "lineSpeed"}, true);
                        if (lineBehavior.typeOfBoxWinning != WinningBoxSequenceType.None)
                        {
                            ShowProperties("Box: ", new string[] { "boxSizeWidth", "boxSizeHeight", "useRaycasterAsBoxPos" }, true);
                        }
                        EditorGUILayout.Space();
                        EditorGUI.indentLevel -= 1;
                        EditorGUILayout.EndVertical();
